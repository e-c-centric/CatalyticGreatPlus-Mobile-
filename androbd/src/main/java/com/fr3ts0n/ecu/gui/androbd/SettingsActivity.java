...
public class SettingsActivity
	...
	public class PrefsFragment
		...
		public void onCreate(Bundle savedInstanceState) {
			...
			Preference logPref = findPreference("log_master");
			if (logPref != null) {
				// logPref.setEnabled(false); // disables interaction
				// logPref.setSelectable(false);
				// logPref.setShouldDisableView(true);
				// logPref.setSummary(""); // optional: blank out summary
				// logPref.setTitle(""); // optional: blank out title
				// logPref.setLayoutResource(0); // optional: remove layout
				// logPref.setVisible(false); // if using AndroidX, otherwise remove from screen:
				getPreferenceScreen().removePreference(logPref); // <-- this removes it from the UI
			}
		...
		void setupElmCmdSelection() {
			MultiSelectListPreference pref = (MultiSelectListPreference) findPreference(ELM_CMD_DISABLE);
		...
		void setupPidSelection() {
			MultiSelectListPreference itemList = (MultiSelectListPreference) findPreference(KEY_DATA_ITEMS);
		...
		void updateNetworkSelections() {
			boolean networkSelected = String.valueOf(CommService.MEDIUM.NETWORK.ordinal())
					.equals(prefs.getString(KEY_COMM_MEDIUM, ""));
			boolean bluetoothSelected = String.valueOf(CommService.MEDIUM.BLUETOOTH.ordinal())
					.equals(prefs.getString(KEY_COMM_MEDIUM, ""));
			boolean usbSelected = String.valueOf(CommService.MEDIUM.USB.ordinal())
					.equals(prefs.getString(KEY_COMM_MEDIUM, ""));
